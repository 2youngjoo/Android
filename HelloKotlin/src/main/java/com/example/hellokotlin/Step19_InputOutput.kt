...
/*
     반복문 3번 돌면서 키보드로  부터 문자열을 각각 3줄을 입력 받아서
...
fun main(){
    ...
    // 반복문 돌면서 메세지 3줄 입력 받기
    for( i in 0..2){
        println("메세지 입력:")
        val msg = scan.nextLine()
        // 입력 받은 문자열을 List에 누적 시키기
        list.add(msg)
    }
    // 문자열을 파일에 출력할 객체
    ...
    // 위의 File 객체를 이용해서 append mode인 FileWriter 객체 얻어내기
    val fw = FileWriter(f, true)
    // 반복문 돌면서 List에 누적된 문자열을 FileWriter 객체를 이용해서 출력하기
    list.forEach{
        fw.append(it+"\n")
    }
    fw.close()
