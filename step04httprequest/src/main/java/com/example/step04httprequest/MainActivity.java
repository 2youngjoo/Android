...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // -> 액티비티가 활성화 되는 시점에 main thread(UI thread)가 여기를 실행한다.
        ...
        requestBtn.setOnClickListener(view->{
            // 1. 입력한 Url 주소를 읽어와서
            // -> 요청 버튼을 누를 때마다 main thread가 여기를 실행한다.
            String Url=inputUrl.getText().toString();
            // 2. http 요청을 하고
            new RequestTask().execute("https://acornacademy.co.kr/index.jsp"); // 이 메소드는 호출 즉시 리턴된다.
            // 3. 정상적으로 응답이되면 응답된 문자열을 EditText 출력하기
        });
    }
    // 비동기 Task 객체를 생성할 클래스
    class RequestTask extends AsyncTask<String, Void, String>{

        // 백그라운드(새로운 스레드의 run() 메소드)에서 작업할 내용을 이 메소드에서 작업하면 된다.
        @Override
        ...
        // doInBackground() 메소드에서 리턴된 문자열이 이 메소드가 호출 되면서 메소드의 인자로 전달된다.
        @Override
